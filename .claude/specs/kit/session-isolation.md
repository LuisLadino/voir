...
**The write-side defense — `sync-kit.sh` repairs gitignored manifest files (#891).** A downstream `.gitignore` with an unanchored `build/` matches `.claude/skills/build/` at any depth, so a synced manifest file can be silently uncommittable: on disk, invisible to `git status` (porcelain excludes ignored files), never reaching committed `main` or any worktree. Reproduced in space-names 2026-07-04 — the sync reported `+1 added` while git ignored the file. After each git-target sync, `enforce_committable_kit_files` feeds the manifest through one `git check-ignore -v --stdin` batch (it consults the index, so already-tracked files are never reported). Repair is provenance-gated: only rules from a working-tree `.gitignore` — the accidental-collision class — are `git add -f`ed, a one-time durable repair since gitignore does not apply to tracked files, with a loud warning naming the rule and the anchor fix (`build/` → `/build/`). Rules from `.git/info/exclude` or a global excludes file are deliberate operator choices: warned, never staged. This provenance gate also makes the client-mode skip defense-in-depth rather than the sole guard — client mode's exclusion lives in `info/exclude`, which is never auto-repaired even if the client marker is lost. Skipped in dry-run (guarded inside the function) and for client-mode targets (guarded at the call site).

**The write-side propagation — `sync-kit.sh` manages the runtime-state ignore rules (#894).** The kit creates transient per-machine state under `.claude/` (dispatch workers, worktrees, session markers, skill-runtime, sync-state and other caches). The kit ignores these in its own repo, but that rule never propagated downstream (#186's DoD claimed it but no mechanism existed), so any downstream running `/dispatch` committed transient state — observed in space-names, which had `.claude/specs/.sync-state.json` tracked. After each non-client git-target sync, `setup_managed_gitignore` writes a conda-init-style sentinel block (`# >>> claude-kit managed >>>` … `# <<< claude-kit managed <<<`) into the downstream's **committed** `.gitignore` — committed, not `.git/info/exclude`, so every worktree inherits it — and `git rm --cached`es any already-committed matches (kept on disk, they simply leave the index). The block is found-and-replaced, never blind-appended (naive append is non-idempotent, conda#8703), so re-runs converge with no churn and user content outside the block is untouched; a lone begin marker re-emits its buffered lines rather than swallowing the file. The pattern list is a curated set of kit-created transient paths, deliberately **not** a copy of the kit's own `.gitignore .claude/` block: `.claude/.kit-manifest` must stay tracked (this very hook and every worktree depend on it) and `.claude/agent-memory/` is persistent, so a mirror would un-track both. Skipped in dry-run and for client-mode targets — a client `.claude/` is already excluded via `.git/info/exclude`, and client-mode.md forbids `.claude/` patterns in a client's committed `.gitignore`; the skip is enforced both at the call site (CLIENT_PROJECTS membership) and inside the function (`is_client_marked`, defense-in-depth for a marked-but-unlisted single-target sync).
...
- **CANNOT COMMIT** — manifest files on disk but untracked and gitignored (#891). They can never be committed, so `main` and every worktree silently miss them while the two signals above stay quiet: porcelain excludes ignored files (blinding UNCOMMITTED) and on-disk content matches the kit (blinding BEHIND). Detected via one `git check-ignore -v --stdin` batch, reported with the offending rule, and excluded from BEHIND so it is never mislabeled a "committed file that differs". Pure-local, needs no kit. Skipped in client-mode repos (`.claude/kit-mode.yaml` reads `mode: client`): their `.claude/` is excluded from commits by design, so every manifest file would false-flag and the batch would pay a measured ~4s per-session tax at fleet manifest size.
...
- `sync-kit.sh` (`setup_managed_gitignore`). Layer 6 write-side propagation (#894). Ensures the kit's runtime-state ignore rules live in a non-client downstream's committed `.gitignore` via an idempotent sentinel block, and untracks already-committed matches with `git rm --cached`. Curated pattern list, not a mirror of the kit's own ignores. Client-skipped at call site and inside the function.
...
- `sync-kit.sh` never commits or pushes; it writes a downstream's working tree and reports propagation state. It makes two kinds of index mutation, both staging-as-repair that the downstream's own session then commits: `git add -f` on a manifest file that gitignore would otherwise make uncommittable (#891), and `git rm --cached` on committed runtime-state files that the managed `.gitignore` block now ignores (#894). The commit is always the downstream's own session via `/kit-sync` then `/commit`. The kit is the source of truth and re-sync regenerates downstream copies, so a lagging downstream loses nothing.
...
`.claude/hooks/context/kit-drift-warning.test.cjs` covers the UNCOMMITTED and BEHIND signals (changed, added, removed upstream), the CANNOT COMMIT signal (#891: gitignored manifest file flagged with its rule, excluded from BEHIND via a mutation-killing differing-content fixture, anchored rule not flagged, client-mode skip, non-git degrade), the manifest-scoped dirty filter, the no-kit-source fallback, the not-a-downstream and silence/no-framework paths, and `resolveKitSource` rejecting a downstream.
`scripts/sync-kit-gitignore-defense.test.sh` covers the sync-side #891 defense: force-add + loud warning + propagation co-fire on an unanchored `build/`, no-op on anchored rules, `info/exclude` warned-but-never-staged (provenance gate), client-mode and non-git skips, dry-run index safety.
`scripts/sync-kit-managed-gitignore.test.sh` covers the #894 managed-block propagation: block creation, user-content preservation, untrack of committed runtime state, idempotent re-sync (byte-identical, no dupes), curation (`.kit-manifest`/`agent-memory/` stay trackable), stale-block rewrite, lone-marker self-heal (no data loss), client-mode (listed and marker-only) and non-git skips, dry-run index safety.
